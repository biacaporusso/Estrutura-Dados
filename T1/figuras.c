...
Figura criarFig(char tipo[],char id[], double x, double x1, double x2, double y, double y1, double y2, double r, double w, double h, char borda[], char preench[], char textoGeo[], char ancora[]) {
    ...
    sprintf(fig->corb, "%s", borda==NULL?"none":borda);
    sprintf(fig->corp, "%s", preench==NULL?"none":preench);
    sprintf(fig->txto, "%s", textoGeo==NULL?"":textoGeo);
    sprintf(fig->ancora, "%s", ancora==NULL?"none":ancora);
